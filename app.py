...
def init()->tuple:
    ...
    parser.add_argument('--preload_data', default=False, help='use preload data or download data from world_bank/oecd via api')
    parser.add_argument('--use_postgres', default=False, help='Use or not postgres for storing data')
    parser.add_argument('--postgres_username', type=str, default=None, help='Username for postgres (required if `USE_POSTGRES`=True)')
    parser.add_argument('--postgres_password', type=str, default=None, help='Password for postgres (required if `USE_POSTGRES`=True)')
    parser.add_argument('--postgres_port', type=int, default=None, help='Port for postgres (required if `USE_POSTGRES`=True)')
    parser.add_argument('--use_ollama', default=False, help='Use or not ollama llm for description of economic situation')
    parser.add_argument('--ollama_model', type=str, default=None, help='Model name of ollama llm for use')
    parser.add_argument('--web_ui_port', type=int, default=None, help='Port for web application with dashboard')
...
def prepare_data(CONFIG:dict, METADATA:dict)->tuple:
    ...
    if CONFIG['USE_POSTGRES']:
        assert isinstance(CONFIG['POSTGRES_USERNAME'], str), logging.error('You should provide `POSTGRES_USERNAME` in order to use postgres as database')
        assert isinstance(CONFIG['POSTGRES_PASSWORD'], str), logging.error('You should provide `POSTGRES_PASSWORD` in order to use postgres as database')
        assert isinstance(CONFIG['POSTGRES_PORT'], str|int), logging.error('You should provide `POSTGRES_PORT` in order to use postgres as database')
        ...
        if CONFIG['PRELOAD_DATA']:
            ...
            assert set(METADATA.keys()).issubset(set(tables)), logging.error('Metadata is not compatible with datasets')
